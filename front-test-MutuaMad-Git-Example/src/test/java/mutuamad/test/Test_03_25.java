...
public class Test_03_25 extends CreateExtetReport {
    ...
    public void Dar_Parte_Verificar_Datos_Guardado() throws Exception {
        ...
        Utilities.sleep(2000);
        implicityScreen.selectNoVehiclePlateOptionDropDownMenu();
        implicityScreen.selectNoInsuranceOptionDropDownMenu();
        Utilities.sleep(2000);
        implicityScreen.clickbuttonContinue();
        ...
        reportsScreen.closeReportScreen();
        reportsScreen.clickButtonSavedReportCancelarToClose();
        //Return give part Saved Previously
        welcomeScreen.clickButtonReports();
        reportsScreen.clickButtonReportSavedPreviously();

        //VERIFY that the part has been updated
        CreateExtetReport.stepsVerifications(reportsScreen.verifyReportOptionInformedIsShown(), test1, "update report");
        Assert.assertTrue(reportsScreen.verifyReportOptionInformedIsShown(), "Los datos del parte no se han actualizado");
        ...
        reportsScreen.closeReportScreen();
        reportsScreen.clickButtonSavedReportAceptarToClose();
        //Return give part Saved Previously
        welcomeScreen.clickButtonReports();
        reportsScreen.clickButtonReportSavedPreviously();
        Utilities.sleep(2000);
        ...
        reportsScreen.closeReportScreen();
        reportsScreen.clickButtonSavedReportAceptarToClose();
        //Return give part Saved Previously for Deleted
        welcomeScreen.clickButtonReports();
        reportsScreen.clickButtonBin();
        reportsScreen.clickButtonDeleteReportSaved();

        //VERIFY that the saved part is not displayed
        CreateExtetReport.stepsVerifications(!reportsScreen.verifyListReportsSavedIsShown(), node3, "update report");
        Assert.assertFalse(reportsScreen.verifyListReportsSavedIsShown(), "Los datos del parte no se han actualizado");
