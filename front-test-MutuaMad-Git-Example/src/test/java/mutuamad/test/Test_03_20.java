...
public class Test_03_20  extends CreateExtetReport {

    @Test
    public void Dar_Parte_Moto_Incendio() throws Exception {
        ...
        //Initialize report
        test1 = extent.createTest(this.getClass().getSimpleName(), "Dar parte, moto, ha habido un incendio");
        ...
        // Open new MotorBike report a Fire
        ...
        Assert.assertFalse(reportsScreen.verifyTeHaOcurridoScreenISShown() && verifyNumberOfPolicy<=0,"Este usuario no tiene permisos para dar parte de incendio");
        ...
        if(verifyNumberOfPolicy>0) {
            for (int x = 1; x <= verifyNumberOfPolicy; x++) {
                ...
                CreateExtetReport.stepsVerifications(x-1 == verifyNumberOfPolicy, test1, "NO report available");
        ...
        CreateExtetReport.stepsVerifications(reportsScreen.verifyThereWasAFireOptionIsShown(), node3, "Choosing the correct claim from a range of options");
        Assert.assertTrue(reportsScreen.verifyThereWasAFireOptionIsShown(),"Este usuario no tiene permisos para dar parte de incendios");
        reportsScreen.clickThereWasAFire();
        Utilities.sleep(2000);
